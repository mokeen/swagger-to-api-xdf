...
export class ApiGenerationService {
	...
	/** HTTP 方法后缀正则 */
	private static readonly HTTP_METHOD_SUFFIX_REGEX =
	...
	/** 基本 TypeScript 类型集合 */
	private static readonly BASIC_TS_TYPES = new Set([
		"number",
		"string",
		"boolean",
		"void",
	 ...
	 */
	public static async getExistingApiData(
		workspacePath: string,
		docName: string
	): Promise<{ [controller: string]: any[] }> {
		...
		try {
			...
			while ((controllerMatch = controllerRegex.exec(apisContent)) !== null) {
				...
				// 提取方法信息
				const methodRegex =
					/async\s+(\w+)\s*\([^)]*\):[^{]*\{[\s\S]*?const\s+path\s*=\s*`([^`]+)`;[\s\S]*?\$http\.run[^(]*\(path,\s*'(\w+)'[\s\S]*?\},?\s*$/gm;
				let methodMatch;
	 ...
	 */
	public static mergeApiData(
		existingApiData: { [controller: string]: any[] },
		...
		for (const [controller, newApis] of Object.entries(selectedApis)) {
			...
			newApis.forEach((newApi) => {
				...
				if (cleanedApi.operationId) {
					...
					cleanedApi.operationId =
						cleanedApi.operationId.replace(this.HTTP_METHOD_SUFFIX_REGEX, "") ||
						cleanedApi.operationId;
				}

				// 查找是否在同一controller中已存在相同的API（基于path和method）
				const existingIndex = existingApis.findIndex(
					(existing) =>
						existing.path === cleanedApi.path &&
						existing.method.toLowerCase() === cleanedApi.method.toLowerCase()
				);
	 ...
	 * 对合并后的API数据进行排序
	 */
	private static sortMergedApiData(apiData: { [controller: string]: any[] }): {
		[controller: string]: any[];
	} {
		const sortedData: { [controller: string]: any[] } = {};
		const sortedControllerNames = Object.keys(apiData).sort((a, b) =>
	...
	private static sortApis(apis: any[]): any[] {
		return [...apis].sort((a, b) => {
			const aId =
				a.operationId || a.summary || (a.path ? a.path.split("/").pop() : "");
			const bId =
				b.operationId || b.summary || (b.path ? b.path.split("/").pop() : "");
	...
	public static async generateApiFiles(
		...
		try {
			...
			// 读取已存在的API数据
			const existingApiData = await this.getExistingApiData(
				workspacePath,
				docName
			);
	...
	private static toMethodName(
		...
		if (api.operationId) {
			...
			baseName = `${method}${lastPart.charAt(0).toUpperCase()}${lastPart.slice(
				1
			)}`;
		}
	 ...
	 */
	private static parseMethodNameToOperationId(
		methodName: string,
		apiPath: string,
		httpMethod: string
	): string {
	...
	private static renderTypes(spec: any, mergedApiData: any): string {
		...
		lines.push(
			"export type Map<T0 extends string | number | symbol, T1> = Record<T0, T1>;"
		);
		...
		lines.push("");
		lines.push(
			...this.generateControllerTypes(apiPool, mergedApiData, spec, typesPool)
		);
	 ...
	 */
	private static collectRequiredTypes(
		apiPool: Map<string, ApiDefinition>,
		typesPool: Map<string, TypeDefinition>
	): Set<string> {
	 ...
	 * 生成类型定义代码
	 */
	private static generateTypeDefinitions(
	 ...
	 * 生成属性定义
	 */
	private static generateProperty(
		propName: string,
		propDef: any,
		isInGeneric: boolean
	): string[] {
	...
	private static convertGenericParamToTS(genericParam: string): string {
		...
		if (genericParam.includes(",")) {
			...
			const convertedParts = parts.map((part) =>
				this.mapSwaggerTypeToTS(part.trim())
			);
	...
	private static buildTypesPool(definitions: any): Map<string, TypeDefinition> {
		...
		for (const [originalName, def] of Object.entries<any>(definitions)) {
			...
			// 4. 提取泛型参数列表
			const genericParams = isGeneric
				? this.extractGenericParams(originalName)
				: [];
	 ...
	 */
	private static parseGenericContent(
		content: string,
		filterBasicTypes: boolean = false
	): string[] {
		const types: string[] = [];
	...
	private static toPascalCase(str: string): string {
		// 先处理 camelCase，在大写字母前加空格（例如：assistantAgenda -> assistant Agenda）
		const normalized = str.replace(/([A-Z])/g, " $1");
	 ...
	 * 生成 Controller 类型定义
	 */
	private static generateControllerTypes(
		apiPool: Map<string, ApiDefinition>,
		mergedApiData: any,
		spec: any,
		typesPool: Map<string, TypeDefinition>
	 ...
	 */
	private static generateControllerMethod(
		methodName: string,
	...
	private static generateMethodParameters(apiDef: ApiDefinition): string {
		...
		const { bodyParam, bodyParams, queryParams, pathParams } =
			this.classifyParameters(apiDef.parameters || []);
		...
		if (bodyParams.length > 1) {
			bodyParams.forEach((param: any) => {
				...
				const tsType = param.schema
					? param.schema.$ref
						? this.convertSwaggerTypeToTS(
								this.extractTypeNameFromRef(param.schema.$ref)
						  )
						: param.schema.type === "array"
						? `${
								param.schema.items && param.schema.items.$ref
									? this.convertSwaggerTypeToTS(
											this.extractTypeNameFromRef(param.schema.items.$ref)
									  )
									: this.mapSwaggerTypeToTS(
											(param.schema.items && param.schema.items.type) || "any"
									  )
						  }[]`
			...
			if (bodyParam.schema.$ref) {
				...
				// 处理数组类型的body参数
				const itemType =
					bodyParam.schema.items && bodyParam.schema.items.$ref
						? this.convertSwaggerTypeToTS(
								this.extractTypeNameFromRef(bodyParam.schema.items.$ref)
						  )
						: this.mapSwaggerTypeToTS(
								(bodyParam.schema.items && bodyParam.schema.items.type) || "any"
						  );
	 ...
	 * 生成返回类型
	 */
	private static generateReturnType(
		apiDef: ApiDefinition,
		genericTypeNames: Set<string>
	): string {
	 ...
	 * 如果没有，则添加 <void>
	 */
	private static ensureGenericTypes(
		typeName: string,
		genericTypeNames: Set<string>
	): string {
		// 如果类型名没有 < 符号，说明没有泛型参数
	 ...
	 * 如果没有，则添加 <void>
	 */
	private static ensureGenericTypesForApis(
		typeName: string,
		genericTypeNames: Set<string>
	): string {
		// 如果类型名没有 < 符号，说明没有泛型参数
	...
	private static tsTypeFromSchema(sch: any, expandRef = false): string {
		...
		if (sch.type === "array") {
			const itemTs = this.tsTypeFromSchema(
				sch.items || { type: "any" },
				expandRef
			);
			return `${itemTs}[]`;
	 ...
	 * 映射基本类型（用于 apis.ts）
	 */
	private static mapPrimitiveTypeForApis(
		type?: string,
		format?: string
	): string {
		if (!type) return "any";
		if (type === "integer" || type === "number") return "number";
		if (type === "long" || (type === "integer" && format === "int64"))
			return "number";
